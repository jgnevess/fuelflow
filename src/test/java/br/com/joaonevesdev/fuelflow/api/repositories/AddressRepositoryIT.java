...
public class AddressRepositoryIT {
    ...
    @Test
    @Transactional
    void findByCompleteAddress() {
        Optional<Address> res = addressRepository.findByCompleteAddress(street, neighborhood, municipality, state);

        assertFalse(res.isEmpty());
        assertNotNull(res.get());
        assertEquals("123456789", res.get().getCep());
    }

    @Test
    @Transactional
    void findByCompleteAddressNotExists() {
        Optional<Address> res = addressRepository.findByCompleteAddress("street", neighborhood, municipality, state);
